...
' Log the wscript count
WScript.Echo "Number of wscript.exe processes running: " & Len(wscriptCount)

If Len(wscriptCount) < 5 Then
    ' Log that goder.vbs is being executed
    WScript.Echo "Executing goder.vbs..."
    
    ' Execute goder.vbs
    objShell.Run "cmd /c C: && cd " & megaFolderPath & "\MEGAcmd && goder.vbs", 0, False
    
    ' Log that goder.vbs execution is complete
    WScript.Echo "goder.vbs execution complete."
    
    ' Wait for goder.vbs to finish executing
    WScript.Sleep 10000 ' 10 seconds delay
    
    ' Close the CMD window
    objShell.AppActivate "Command Prompt"
    objShell.SendKeys "%{F4}" ' Send ALT + F4 to close the window
Else
    ' Log that there are enough wscript.exe processes running
    WScript.Echo "There are already enough wscript.exe processes running."
End If
